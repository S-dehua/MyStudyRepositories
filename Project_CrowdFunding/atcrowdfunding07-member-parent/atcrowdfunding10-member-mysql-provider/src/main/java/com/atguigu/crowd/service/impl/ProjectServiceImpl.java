...
public class ProjectServiceImpl implements ProjectService {
	...
	public void saveProject(ProjectVO projectVO, Integer memberId) {
		...
		// 修复bug：把money放入（类型不同无法赋值）
		projectPO.setMoney(Long.parseLong(projectVO.getMoney().toString()));
		...
		// 修复bug：生成结束时间
		// String deploydate = new SimpleDateFormat("yyyy-MM-dd").format(new Date(date.getTime() - (3 * 24 * 60 * 60 *1000)));
		Calendar calendar = Calendar.getInstance();
		calendar.add(Calendar.DAY_OF_YEAR, projectPO.getDay());
		String deploydate = new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime());
